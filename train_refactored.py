...
from models.efficientdet import build_efficientdet
from datasets import VOCDetection, CocoDataset, Resizer, Normalizer, Augmenter, collater
...
parser.add_argument(
    "--network", default="efficientdet-d0", type=str, help="efficientdet-[d0, d1, ..]"
)
...
parser.add_argument("--num_epoch", default=500, type=int, help="Num epoch for training")
parser.add_argument(
    "--batch_size", default=32, type=int, help="Batch size for training"
)
parser.add_argument(
    "--num_class", default=20, type=int, help="Number of class used in model"
)
parser.add_argument(
    "--device", default=[0, 1], type=list, help="Use CUDA to train model"
)
parser.add_argument(
...
parser.add_argument(
    "--seed", default=24, type=int, help="seed for initializing training. "
)
...
def train(train_loader, model, scheduler, optimizer, epoch, args):
    ...
    for idx, (images, annotations) in enumerate(train_loader):
        ...
        classification_loss, regression_loss = model.calculate_loss(
            [images, annotations]
        )
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = build_efficientdet(args.network, args.num_classes)
    if args.resume is not None:
    ...
    for epoch in range(args.start_epoch, args.num_epoch):
        ...
        state = {"epoch": epoch, "parser": args, "state_dict": get_state_dict(model)}
