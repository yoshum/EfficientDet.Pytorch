...
parser.add_argument(
    "-n", "--network", default="efficientdet-d0", help="efficientdet-[d0, d1, ..]"
)
parser.add_argument(
    "-s", "--score", default=True, action="store_true", help="Show score"
)
parser.add_argument(
    "-t", "--threshold", default=0.6, type=float, help="Visualization threshold"
)
parser.add_argument(
    "-it", "--iou_threshold", default=0.6, type=float, help="Visualization threshold"
)
parser.add_argument(
    "-w",
    "--weight",
    default="./weights/voc0712.pth",
    type=str,
    help="Weight model path",
)
parser.add_argument("-c", "--cam", action="store_true", help="Use camera")
parser.add_argument("-f", "--file_name", default="pic.jpg", help="Image path")
parser.add_argument(
    "--num_class", default=21, type=int, help="Number of class used in model"
)
...
class Detect(object):
    ...
    def __init__(
        self, weights, num_class=21, network="efficientdet-d0", size_image=(512, 512)
    ):
        ...
        with torch.no_grad():
            ...
            for j in range(scores.shape[0]):
                ...
                if args.score:
                    ...
                    if args.cam:
                        labelSize, baseLine = cv2.getTextSize(
                            "{}".format(label_name), cv2.FONT_HERSHEY_SIMPLEX, 0.8, 2
                        )
                        cv2.rectangle(
                            origin_img,
                            (x1, y1 - labelSize[1]),
            ...
            if show:
                fig, ax = vis_bbox(
                    img=origin_img, bbox=bboxes, label=labels, score=bbox_scores
                )
